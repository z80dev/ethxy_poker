...
# _name: constant(String[24]) = "L1 Test Token"
# _symbol: constant(String[5]) = "L1TT"
...
@external
def __init__(name: String[24], symbol: String[5], initialSupply: uint256):
    self.name = name
    self.symbol = symbol
...
def permit(owner: address, spender: address, amount: uint256, deadline: uint256, v: uint8, r: bytes32, s: bytes32):
    ...
    domain_separator: bytes32 = keccak256(
        _abi_encode(_DOMAIN_TYPEHASH, keccak256(self.name), keccak256("1.0"), chain.id, self)
    )

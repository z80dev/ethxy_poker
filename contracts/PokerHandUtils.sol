...
contract PokerHandUtils {
	...
	function getCardCode(CardValue value, CardSuit suit) public pure returns (int8) {
		return int8(int256(uint256(suit)))*13 + int8(int256(uint256(value)));
	}
	...
	function getCardOrderValue(CardValue cardVal) pure public returns (int8)
	{
		...
		if (int8(int(uint(cardVal))) == 0) {
			return int8(int256(uint256(CardValue.Ace_High)));
		}
		return int8(int(uint(cardVal)));
	}

	// Helper Sort function
	// Modified from
	// https://github.com/alice-si/array-booster
	function sortHand(int8[5] memory data) pure public {
		uint n = data.length;
